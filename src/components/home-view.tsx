...
import type { Dress, Message, Invoice, Collection, User, TabId } from "@/types";
import { STATUS, GREEN, RED, AMBER, MUTED, GOLD } from "@/constants";
...
export const HomeView = ({ dresses, messages, invoices, collections, user, setTab, setColFilter }: HomeViewProps) => {
  const delayed = dresses.filter(d => d.status === "delayed");
  const needsReview = messages.filter(m => m.needsReview && !m.resolved).length;
  const activeInvoices = invoices.filter(i => i.shippingStatus && i.shippingStatus !== "Delivered");
    ...
    <div className="fade-up">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 28 }}>
        {[
          ...
          { l: "In Transit", v: activeInvoices.length, c: "var(--text)" },
        ].map(({ l, v, c }) => (
                ...
                <Card key={col.id} onClick={() => { setColFilter(col.id); setTab("dresses"); }} style={{ padding: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    <div style={{ position: "relative", flexShrink: 0 }}>
                      <Ring value={onTrack} total={Math.max(total, 1)} color={GOLD} size={56} stroke={4} />
                      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                        <span style={{ fontSize: 15, fontWeight: 800, lineHeight: 1 }}>{onTrack}</span>
                        <span style={{ fontSize: 8, color: "var(--text3)" }}>/{total}</span>
                      </div>
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: "var(--font-display)", fontSize: 16, fontWeight: 600 }}>{col.name}</div>
                      {col.launchDate && <div style={{ fontSize: 10, color: "var(--text3)", marginTop: 2 }}>Launch: {fmtDate(col.launchDate)}</div>}
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
                        ...
                        <span style={{ fontSize: 9, color: MUTED, background: "var(--surface2)", borderRadius: 20, padding: "1px 8px" }}>{total} styles</span>
                      </div>
                    </div>
                    <div style={{ color: "var(--border2)", fontSize: 18 }}>{"\u203a"}</div>
                  </div>
                  {total > 0 && (
                    <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(total, 20)},1fr)`, gap: 3, marginTop: 10 }}>
                      {cd.slice(0, 20).map(d => {
                        const c = STATUS[d.status]?.color || MUTED;
                        return <div key={d.id} style={{ height: 5, borderRadius: 3, background: c }} />;
                      })}
                    </div>
                  )}
                </Card>
